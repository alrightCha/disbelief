...
var getSqrtPriceFromPrice = (price, tokenADecimal, tokenBDecimal) => {
  ...
  return new BN5(sqrtValueQ64.floor().toFixed());
};
...
var getLiquidity = (baseAmount, quoteAmount, minSqrtPrice, maxSqrtPrice) => {
  ...
  return BN5.min(liquidityFromBase, liquidityFromQuote);
};
...
var getSwapAmountWithBuffer = (swapBaseAmount, sqrtStartPrice, curve) => {
  const swapAmountBuffer = swapBaseAmount.add(
    swapBaseAmount.mul(new BN5(25)).div(new BN5(100))
  );
  ...
  return BN5.min(swapAmountBuffer, maxBaseAmountOnCurve);
};
var calculatePercentageSupplyOnMigration = (initialMarketCap, migrationMarketCap, lockedVesting, totalTokenSupply) => {
  ...
  const totalVestingAmount = getTotalVestingAmount(lockedVesting);
  const vestingPercentageDecimal = new Decimal(totalVestingAmount.toString()).mul(new Decimal(100)).div(new Decimal(totalTokenSupply.toString()));
  const vestingPercentage = vestingPercentageDecimal.toNumber();
  const numerator = new Decimal(100).mul(sqrtRatio).sub(new Decimal(vestingPercentage).mul(sqrtRatio));
...
var calculateMigrationQuoteThreshold = (migrationMarketCap, percentageSupplyOnMigration) => {
  ...
  return migrationMarketCapDecimal.mul(percentageDecimal).div(new Decimal(100)).toNumber();
};
...
function getBaseFeeNumerator(feeSchedulerMode, cliffFeeNumerator, period, reductionFactor) {
  ...
  if (feeSchedulerMode == 0 /* Linear */) {
    ...
    const bps = reductionFactor.shln(OFFSET).div(new BN5(BASIS_POINT_MAX));
...
function getDynamicFeeParams(baseFeeBps, maxPriceChangeBps = MAX_PRICE_CHANGE_BPS_DEFAULT) {
  ...
  const sqrtPriceRatioQ64 = new BN5(
    Decimal.sqrt(priceRatio.toString()).mul(Decimal.pow(2, 64)).floor().toFixed()
  );
...
async function getAccountData(accountAddress, accountType, program) {
  const address = accountAddress instanceof PublicKey2 ? accountAddress : new PublicKey2(accountAddress);
...
function deriveDammV1EventAuthority() {
  const [eventAuthority] = PublicKey3.findProgramAddressSync(
...
function deriveDbcPoolAuthority() {
  const [poolAuthority] = PublicKey3.findProgramAddressSync(
...
function deriveDbcPoolAddress(quoteMint, baseMint, config) {
  const isQuoteMintBiggerThanBaseMint = new PublicKey3(quoteMint).toBuffer().compare(new Uint8Array(new PublicKey3(baseMint).toBuffer())) > 0;
  const [pool] = PublicKey3.findProgramAddressSync(
    [
      Buffer.from(SEED.POOL),
      new PublicKey3(config).toBuffer(),
      isQuoteMintBiggerThanBaseMint ? new PublicKey3(quoteMint).toBuffer() : new PublicKey3(baseMint).toBuffer(),
      isQuoteMintBiggerThanBaseMint ? new PublicKey3(baseMint).toBuffer() : new PublicKey3(quoteMint).toBuffer()
    ],
    DYNAMIC_BONDING_CURVE_PROGRAM_ID
  );
  return pool;
...
function deriveMintMetadata(mint) {
  const [metadata] = PublicKey3.findProgramAddressSync(
    [
...
function derivePartnerMetadata(feeClaimer) {
  const [partnerMetadata] = PublicKey3.findProgramAddressSync(
...
function deriveDbcTokenVaultAddress(pool, mint) {
  const [tokenVault] = PublicKey3.findProgramAddressSync(
...
var deriveVaultPdas = (tokenMint, seedBaseKey) => {
  ...
  const [lpMint] = PublicKey3.findProgramAddressSync(
...
function deriveEscrow(base) {
  const [escrow] = PublicKey3.findProgramAddressSync(
...
function validatePoolFees(poolFees) {
  ...
  if (poolFees.baseFee) {
    if (poolFees.baseFee.cliffFeeNumerator.lte(new BN6(0))) {
      return false;
    }
  }
  return true;
...
function validateCurve(curve, sqrtStartPrice) {
  ...
  if (curve[0]?.sqrtPrice.lte(sqrtStartPrice) || curve[0]?.liquidity.lte(new BN6(0)) || curve[0]?.sqrtPrice.gt(new BN6(MAX_SQRT_PRICE))) {
    return false;
  }
  for (let i = 1; i < curve.length; i++) {
    ...
    if (currentPoint.sqrtPrice.lte(previousPoint.sqrtPrice) || currentPoint.liquidity.lte(new BN6(0))) {
      return false;
    }
  }
  return !curve[curve.length - 1]?.sqrtPrice.gt(new BN6(MAX_SQRT_PRICE));
}
function validateTokenSupply(tokenSupply, leftoverReceiver, swapBaseAmount, migrationBaseAmount, lockedVesting, swapBaseAmountBuffer) {
  ...
  if (!(leftoverReceiver instanceof PublicKey4)) {
    return false;
  }
  if (leftoverReceiver.equals(PublicKey4.default)) {
    return false;
  }
  ...
  return !(minimumBaseSupplyWithoutBuffer.gt(
    new BN6(tokenSupply.postMigrationTokenSupply)
  ) || new BN6(tokenSupply.postMigrationTokenSupply).gt(
    new BN6(tokenSupply.preMigrationTokenSupply)
...
function validateConfigParameters(configParam) {
  ...
  if (configParam.migrationQuoteThreshold.lte(new BN6(0))) {
    throw new Error("Migration quote threshold must be greater than 0");
  }
  if (new BN6(configParam.sqrtStartPrice).lt(new BN6(MIN_SQRT_PRICE)) || new BN6(configParam.sqrtStartPrice).gte(new BN6(MAX_SQRT_PRICE))) {
    ...
    try {
      const totalAmount = configParam.lockedVesting.cliffUnlockAmount.add(
        configParam.lockedVesting.amountPerPeriod.mul(
          new BN6(configParam.lockedVesting.numberOfPeriod)
        )
      );
      if (configParam.lockedVesting.frequency.eq(new BN6(0)) || totalAmount.eq(new BN6(0))) {
  ...
  if (configParam.tokenSupply) {
    ...
    if (!validateTokenSupply(
      ...
      new PublicKey4(configParam.leftoverReceiver),
      swapBaseAmount,
      migrationBaseAmount,
...
async function validateBalance(connection, owner, inputMint, amountIn, inputTokenAccount) {
  ...
  if (isSOLInput) {
    ...
    try {
      ...
      const balance = new BN6(tokenBalance.value.amount);
...
function buildCurve(buildCurveParam) {
  ...
  const migrationBaseSupply = new BN7(totalTokenSupply).mul(new BN7(percentageSupplyOnMigration)).div(new BN7(100));
  const totalSupply = new BN7(totalTokenSupply).mul(
    new BN7(10).pow(new BN7(tokenBaseDecimal))
  );
  const migrationQuoteThresholdWithDecimals = new BN7(
    migrationQuoteThreshold * 10 ** tokenQuoteDecimal
  );
  const migrationPrice = new Decimal2(migrationQuoteThreshold.toString()).div(
    new Decimal2(migrationBaseSupply.toString())
  );
  const totalLeftover = new BN7(leftover).mul(
    new BN7(10).pow(new BN7(tokenBaseDecimal))
  );
  ...
  const migrationBaseAmount = getMigrationBaseToken(
    new BN7(migrationQuoteThresholdWithDecimals),
    migrateSqrtPrice,
    migrationOption
  );
  const totalVestingAmount = getTotalVestingAmount(lockedVesting);
  ...
  const instructionParams = {
    poolFees: {
      baseFee: {
        ...
        numberOfPeriod,
        reductionFactor: new BN7(reductionFactor),
        periodFrequency: new BN7(periodFrequency),
        feeSchedulerMode
      },
...
function buildCurveByMarketCap(buildCurveByMarketCapParam) {
  ...
  const percentageSupplyOnMigration = calculatePercentageSupplyOnMigration(
    new BN7(initialMarketCap),
    new BN7(migrationMarketCap),
    lockedVesting,
    new BN7(totalTokenSupply)
  );
...
function buildCurveGraph(buildCurveGraphParam) {
  ...
  let priceRatio = new Decimal2(pMax.toString()).div(
    new Decimal2(pMin.toString())
  );
  let qDecimal = priceRatio.pow(new Decimal2(1).div(new Decimal2(16)));
  let sqrtPrices = [];
  let currentPrice = pMin;
  for (let i = 0; i < 17; i++) {
    sqrtPrices.push(currentPrice);
    currentPrice = convertDecimalToBN(
      qDecimal.mul(new Decimal2(currentPrice.toString()))
    );
  }
  let totalSupply = new BN7(totalTokenSupply).mul(
    new BN7(10).pow(new BN7(tokenBaseDecimal))
  );
  let totalLeftover = new BN7(leftover).mul(
    new BN7(10).pow(new BN7(tokenBaseDecimal))
  );
  ...
  let sumFactor = new Decimal2(0);
  let pmaxWeight = new Decimal2(pMax.toString());
  for (let i = 1; i < 17; i++) {
    let pi = new Decimal2(sqrtPrices[i].toString());
    let piMinus = new Decimal2(sqrtPrices[i - 1].toString());
    let k = new Decimal2(liquidityWeights[i - 1]);
  ...
  let l1 = new Decimal2(totalSwapAndMigrationAmount.toString()).div(sumFactor);
  let curve = [];
  for (let i = 0; i < 16; i++) {
    let k = new Decimal2(liquidityWeights[i]);
...
async function getTokenDecimals(connection, mintAddress) {
  const mintPubkey = mintAddress instanceof PublicKey5 ? mintAddress : new PublicKey5(mintAddress);
...
// src/services/pool.ts
import { NATIVE_MINT as NATIVE_MINT3, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID3 } from "@solana/spl-token";
...
function getFeeInPeriod(cliffFeeNumerator, reductionFactor, period) {
  ...
  if (period === 1) {
    const basisPointMax2 = new BN8(BASIS_POINT_MAX);
    return mulDiv(
      cliffFeeNumerator,
...
function getCurrentBaseFeeNumerator(baseFee, currentPoint, activationPoint) {
  ...
  if (feeSchedulerMode === 0 /* Linear */) {
    const reduction = SafeMath.mul(new BN8(period), baseFee.reductionFactor);
...
function getFeeOnAmount(amount, poolFees, isReferral, currentPoint, activationPoint, volatilityTracker) {
  ...
  if (totalFeeNumerator.gt(new BN8(MAX_FEE_NUMERATOR))) {
    totalFeeNumerator = new BN8(MAX_FEE_NUMERATOR);
  }
  const tradingFee = mulDiv(
    amount,
    totalFeeNumerator,
    new BN8(FEE_DENOMINATOR),
    0 /* Up */
  );
  const amountAfterFee = SafeMath.sub(amount, tradingFee);
  const protocolFee = mulDiv(
    tradingFee,
    new BN8(poolFees.protocolFeePercent),
    new BN8(100),
    1 /* Down */
  );
  const tradingFeeAfterProtocol = SafeMath.sub(tradingFee, protocolFee);
  let referralFee = new BN8(0);
  if (isReferral) {
    referralFee = mulDiv(
      protocolFee,
      new BN8(poolFees.referralFeePercent),
      new BN8(100),
      1 /* Down */
    );
  }
...
function getVariableFee(dynamicFee, volatilityTracker) {
  ...
  const volatilityTimesBinStep = SafeMath.mul(
    ...
    new BN8(dynamicFee.binStep)
  );
  const squared = SafeMath.mul(volatilityTimesBinStep, volatilityTimesBinStep);
  const vFee = SafeMath.mul(squared, new BN8(dynamicFee.variableFeeControl));
  const scaleFactor = new BN8(1e11);
  const numerator = SafeMath.add(vFee, SafeMath.sub(scaleFactor, new BN8(1)));
...
function getSwapResult(poolState, configState, amountIn, feeMode, tradeDirection, currentPoint) {
  let actualProtocolFee = new BN9(0);
  let actualTradingFee = new BN9(0);
  let actualReferralFee = new BN9(0);
  let actualAmountIn;
...
function getSwapAmountFromBaseToQuote(configState, currentSqrtPrice, amountIn) {
  if (amountIn.isZero()) {
    return {
      outputAmount: new BN9(0),
      nextSqrtPrice: currentSqrtPrice
    };
  }
  let totalOutputAmount = new BN9(0);
...
async function swapQuote(virtualPool, config, swapBaseForQuote, amountIn, slippageBps = 0, hasReferral, currentPoint) {
  ...
  if (slippageBps > 0) {
    const slippageFactor = new BN9(1e4 - slippageBps);
    const denominator = new BN9(1e4);
...
var StateService = class extends DynamicBondingCurveProgram {
  ...
  async getDammV1LockEscrow(lockEscrowAddress) {
    const metadata = await this.program.account.lockEscrow.fetchNullable(
      lockEscrowAddress instanceof PublicKey6 ? lockEscrowAddress : new PublicKey6(lockEscrowAddress)
    );
    return metadata;
  }
  /**
  ...
  async getDammV1MigrationMetadata(poolAddress) {
    const metadata = await this.program.account.meteoraDammMigrationMetadata.fetch(
      poolAddress instanceof PublicKey6 ? poolAddress : new PublicKey6(poolAddress)
    );
    return metadata;
  }
  /**
  ...
  async getPoolsQuoteFeesByConfig(configAddress) {
    const config = configAddress instanceof PublicKey6 ? configAddress : new PublicKey6(configAddress);
  ...
  async getTokenDecimals(mintAddress, tokenType) {
    const mint = mintAddress instanceof PublicKey6 ? mintAddress : new PublicKey6(mintAddress);
    const mintInfo = await getMint2(
      this.program.provider.connection,
      mint,
      this.commitment,
      tokenType === 1 /* Token2022 */ ? TOKEN_2022_PROGRAM_ID2 : TOKEN_PROGRAM_ID3
    );
    return mintInfo.decimals;
  }
};
...
var PoolService = class extends DynamicBondingCurveProgram {
  ...
  prepareSwapParams(swapBaseForQuote, virtualPoolState, poolConfigState) {
    if (swapBaseForQuote) {
      return {
        inputMint: new PublicKey7(virtualPoolState.baseMint),
        outputMint: new PublicKey7(poolConfigState.quoteMint),
      ...
      return {
        inputMint: new PublicKey7(poolConfigState.quoteMint),
        outputMint: new PublicKey7(virtualPoolState.baseMint),
  ...
  async createPoolAndBuy(createPoolBuyParam) {
    ...
    if (buyAmount) {
      ...
      const swapIx = await this.program.methods.swap({
        ...
        outputTokenAccount,
        payer: poolCreator,
        tokenBaseProgram: tokenType === 0 /* SPL */ ? TOKEN_PROGRAM_ID4 : TOKEN_2022_PROGRAM_ID3,
        tokenQuoteProgram: poolConfigState.quoteTokenFlag === 0 /* SPL */ ? TOKEN_PROGRAM_ID4 : TOKEN_2022_PROGRAM_ID3
      }).instruction();
...
var MigrationService = class extends DynamicBondingCurveProgram {
  ...
  async createLocker(createLockerParam) {
    ...
    const tokenProgram = poolConfigState.tokenType === 0 ? TOKEN_PROGRAM_ID5 : TOKEN_2022_PROGRAM_ID4;
  ...
  async migrateToDammV2(migrateToDammV2Param) {
    ...
    const tokenBaseProgram = poolConfigState.tokenType == 0 ? TOKEN_PROGRAM_ID5 : TOKEN_2022_PROGRAM_ID4;
    const tokenQuoteProgram = poolConfigState.quoteTokenFlag == 0 ? TOKEN_PROGRAM_ID5 : TOKEN_2022_PROGRAM_ID4;
    const tx = await this.program.methods.migrationDammV2().accountsStrict({
      ...
      tokenBaseProgram,
      tokenQuoteProgram,
      token2022Program: TOKEN_2022_PROGRAM_ID4,
      systemProgram: SystemProgram3.programId,
      dammEventAuthority
    }).remainingAccounts([
      {
...
var CreatorService = class extends DynamicBondingCurveProgram {
  ...
  async creatorWithdrawSurplus(creatorWithdrawSurplusParam) {
    ...
    const createQuoteTokenAccountIx = createAssociatedTokenAccountIdempotentInstruction3(
      creator,
      tokenQuoteAccount,
      creator,
      poolConfigState.quoteMint,
    ...
    const accounts = {
      ...
      quoteMint: poolConfigState.quoteMint,
      creator,
      tokenQuoteProgram: TOKEN_PROGRAM_ID6
    };
